...
from database.dummy_db import PENDING_DEPOSITS, USER_BALANCES, PRODUCTS, USER_SPENT
...
async def view_single_deposit(callback: CallbackQuery):
    ...
    data = PENDING_DEPOSITS.get(trxid)
    if not data:
        await callback.answer("❌ This request was processed.", show_alert=True)
        return
    text = f"🔍 <b>Deposit Verification</b>\n\n👤 <b>User ID:</b> <code>{data['user_id']}</code>\n🏦 <b>Method:</b> {data['method']}\n🧾 <b>TrxID:</b> <code>{trxid}</code>\n\n<i>Do you want to approve?</i>"
...
async def process_deposit(callback: CallbackQuery, bot: Bot):
    ...
    data = PENDING_DEPOSITS.get(trxid)
    if not data:
        await callback.answer("❌ Processed.", show_alert=True)
        return
    user_id = data['user_id']
    if action == "appdep":
        USER_BALANCES[user_id] = round(USER_BALANCES.get(user_id, 0.0) + 10.0, 2)
        try: await bot.send_message(user_id, f"🎉 <b>Deposit Approved!</b>\nTrxID: <code>{trxid}</code>\n$10.0 added.", parse_mode="HTML")
        except: pass
...
async def process_add_stock(message: Message, state: FSMContext):
    ...
    product['data'].extend(clean_data)
    ...
    await message.answer(f"✅ <b>Stock Added!</b>\nNew Total Stock: {product['stock']}", reply_markup=keyboard, parse_mode="HTML")
...
async def save_new_product(message: Message, state: FSMContext):
    ...
    PRODUCTS[new_prod_id] = {"name": user_data['prod_name'], "price": user_data['prod_price'], "stock": len(clean_data), "data": clean_data}
    await state.clear()
...
async def search_user_result(message: Message, state: FSMContext):
    ...
    text = f"👤 <b>User Details</b>\n\n🆔 <b>ID:</b> <code>{target_uid}</code>\n💰 <b>Balance:</b> ${USER_BALANCES.get(target_uid, 0.0)}\n💸 <b>Spent:</b> ${USER_SPENT.get(target_uid, 0.0)}"
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
...
async def ask_balance_amount(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(action_type=action, target_user=int(target_uid))
...
async def process_broadcast_send(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if prod_id != "none":
        product = PRODUCTS.get(prod_id)
        if product:
            # 🚀 bcbuy_ প্রিফিক্স দেওয়া হলো যাতে মেসেজ এডিটের ঝামেলা না হয়
